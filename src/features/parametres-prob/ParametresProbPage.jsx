...
import { useResponsablesFullQuery, useSaveResponsableMutation, useDeleteResponsableMutation, useDeleteAllProblematiquesMutation } from './queries';
...
export default function ParametresProbPage() {
  ...
  useEffect(() => { if (respQ.data) setRows(respQ.data.map((r) => ({ ...r, dirty: false }))); }, [respQ.data]);

  function addRow() { setRows((prev) => [...prev, { id: tmpIdCounter--, nom: '', dirty: true }]); }
  function updateNom(idx, val) { setRows((prev) => prev.map((r, i) => (i === idx ? { ...r, nom: val, dirty: true } : r))); }
  async function deleteRow(idx) { const r = rows[idx]; try { await deleteMutation.mutateAsync(r.id); setRows((prev) => prev.filter((_, i) => i !== idx)); addToast('Responsable supprimé.', 'success'); } catch (e) { addToast('Erreur : ' + e.message, 'error'); } }
  async function saveAll() { const dirty = rows.filter((r) => r.dirty && r.nom.trim()); if (!dirty.length) { addToast('Aucune modification à enregistrer.', 'info'); return; } setSaving(true); let errors = 0; for (const r of dirty) { try { const result = await saveMutation.mutateAsync({ id: r.id > 0 ? r.id : null, nom: r.nom.trim() }); setRows((prev) => prev.map((row) => (row === r ? { ...row, id: result.id, dirty: false } : row))); } catch { errors++; } } setSaving(false); if (errors) addToast(errors + ' erreur(s).', 'error'); else addToast('Responsables enregistrés ✓', 'success'); }
  async function handleDeleteAll() { try { const n = await deleteAllMutation.mutateAsync(); if (!n) { addToast('Aucune problématique à supprimer.', 'info'); return; } addToast(n + ' problématique(s) supprimée(s) ✓', 'success'); } catch (e) { addToast('Erreur : ' + e.message, 'error'); } }
  function confirmDeleteAll() { if (confirm('Supprimer définitivement toutes les problématiques ? Cette action est irréversible.')) handleDeleteAll(); }
