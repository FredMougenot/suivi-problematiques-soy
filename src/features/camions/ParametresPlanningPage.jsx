...
const CATEGORIES = [
  { key: 'statut_ligne', label: 'Statut ligne', desc: 'Valeurs du champ Statut ligne.', note: 'La valeur "Annulé" applique une barure sur toute la ligne du tableau (style spécial non représentable par les colonnes seules).' },
...
export default function ParametresPlanningPage() {
  ...
  function handleAdd(catKey) { const ordre = (data[catKey] || []).length + 1; addMutation.mutate({ categorie: catKey, ordre }, { onError: (e) => addToast('Erreur : ' + e.message, 'error') }); }
  function handleDelete(id) { deleteMutation.mutate(id, { onSuccess: () => addToast('Valeur supprimée', 'info'), onError: (e) => addToast('Erreur : ' + e.message, 'error') }); }
  function handleSave(catKey, rows) { saveMutation.mutate(rows, { onSuccess: () => { const label = CATEGORIES.find((c) => c.key === catKey)?.label || catKey; addToast(`${label} enregistré ✓`, 'success'); }, onError: (e) => addToast('Erreur : ' + e.message, 'error') }); }

  if (paramsQ.isLoading) return <LoadingOverlay />;

  return (
    <div className="pp-main">
      <div className="sec-h" style={{ marginBottom: 8, paddingLeft: 60 }}>
        <div>
          <div className="sec-t">Paramètres de planification</div>
          <div className="sec-s">Gérez les valeurs et le style graphique appliqué à chaque valeur dans le tableau.</div>
        </div>
      </div>
      ...
      {CATEGORIES.map((cat) => (
        <ParamSection key={cat.key} category={cat} rows={data[cat.key] || []} onAdd={handleAdd} onDelete={handleDelete} onSave={handleSave} saving={saveMutation.isPending} />
      ))}
    </div>
  );
}
