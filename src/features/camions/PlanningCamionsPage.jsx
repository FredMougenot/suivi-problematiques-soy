...
export default function PlanningCamionsPage() {
  ...
  useEffect(() => {
    ...
    context.all.forEach((entry) => {
      ...
      if (nowMs >= limiteMs && r.statut_ligne !== context.vNonLivre) {
        handleFieldChange(idx, 'statut_ligne', context.vNonLivre, true);
      }
    });
  ...
  function buildPayload(idx, extra = {}) {
    ...
    return {
      id: r.id,
      heure_planif: r.heure_planif || slot?.h || '00:00',
      statut_ligne: eff.statutLigne,
      type_camion: eff.typeCamion || null,
      num_arrivee: r.num_arrivee || null,
      num_depart: eff.isBobtail ? null : (r.num_depart || null),
      dest_arrivee: r.dest_arrivee || null,
      destination: eff.destination || null,
      description: eff.description || null,
      statut: eff.statutArrivee || null,
      chargement: eff.isBobtail ? null : (eff.chargement || null),
      heure_reelle: r.heure_reelle || null,
      etat_verification: r.etat_verification || null,
      ...extra,
    };
  }
    ...
    if (saveImmediately) {
      setTimeout(() => {
        setRows((current) => {
          ...
          saveMutation.mutate({ idx, payload }, {
  ...
  async function handleSaveAll() {
    ...
    for (const entry of context.all) {
      ...
      try {
        ...
        await saveMutation.mutateAsync({ idx, payload });
      } catch (e) { errors++; }
    }
  ...
  function handleDeleteRow(idx, id) {
    ...
    deleteMutation.mutate(id, {
      onSuccess: () => {
        setRows((prev) => { const next = { ...prev }; delete next[idx]; return next; });
        addToast('Camion supplémentaire supprimé.', 'info');
      },
      onError: (e) => addToast('Erreur: ' + e.message, 'error'),
    });
  }
  ...
  function handleAddCamion({ heure, type, numero, dest }) {
    ...
    const newRow = { slot_index: newIdx, heure_planif: heure, type_camion: type || null, destination: dest || null };
    setRows((prev) => ({ ...prev, [newIdx]: newRow }));
    setModalOpen(false);
    setTimeout(() => {
      const payload = { heure_planif: heure, type_camion: type || null, num_arrivee: numero || null, destination: dest || null, statut_ligne: params['statut_ligne']?.[0]?.valeur || 'ACTIF' };
      saveMutation.mutate({ idx: newIdx, payload }, { onError: (e) => addToast('Erreur: ' + e.message, 'error') });
    }, 0);
  }
  ...
  return (
    <div className="tool-main">
      ...
      <div style={{ position: 'relative' }} ref={outerRef}>
        <CamionsTable
          dateStr={dateStr} rows={rows} params={params} context={context} role={role}
          onFieldChange={handleFieldChange} onDeleteRow={handleDeleteRow} rowRefs={rowRefs}
        />
        <TimelineLine isToday={isToday} rows={rows} all={context.all} rowRefs={rowRefs} outerRef={outerRef}
          virtualMin={virtualMin} setVirtualMin={setVirtualMin} />
      </div>
