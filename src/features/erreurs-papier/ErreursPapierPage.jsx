        ...
        {rowsQ.isLoading ? (
          ...
          <>
            <div className="ep-grid-3">
              <div className="panel">
                <div className="panel-header" style={{ '--panel-accent': 'var(--ruby)' }}>
                ...
                <div className="panel-body">
                  <div className="ep-kpi-value" style={{ color: totalErrors > 0 ? 'var(--ruby)' : 'var(--emerald)' }}>{totalErrors}</div>
                  <div className="ep-kpi-sub">{totalErrors === 0 ? 'Aucune erreur détectée' : totalErrors === 1 ? '1 erreur détectée' : totalErrors + ' erreurs détectées'}</div>
                </div>
              </div>
              <div className="panel">
                ...
                <div className="panel-body">
                  <div className="ep-kpi-value" style={{ color: 'var(--text-primary)' }}>{totalCamions}</div>
                  <div className="ep-kpi-sub">{totalCamions === 0 ? 'Aucun camion' : totalCamions === 1 ? '1 camion comptabilisé' : totalCamions + ' camions comptabilisés'}</div>
                </div>
              </div>
              <div className="panel">
                <div className="panel-header" style={{ '--panel-accent': 'var(--copper)' }}>
                ...
                <div className="panel-body">
                  <div className="ep-kpi-value" style={{ color: avgEfficiency !== null && avgEfficiency >= 95 ? 'var(--emerald)' : 'var(--copper)' }}>{avgEfficiency !== null ? avgEfficiency + '%' : '—'}</div>
                  <div className="ep-kpi-sub">{avgEfficiency !== null ? (avgEfficiency >= 95 ? 'Performance excellence' : avgEfficiency >= 85 ? 'Performance très bonne' : avgEfficiency >= 70 ? 'Performance bonne' : avgEfficiency >= 50 ? 'Performance moyenne' : 'Performance critique') : 'Non calculable'}</div>
                </div>
              </div>
            </div>
            ...
            <div className="ep-grid-3" style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(340px,1fr))' }}>
