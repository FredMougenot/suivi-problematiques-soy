  ...
  if (errors.length === 0) {
    return (
      <div className="error-none">
        ...
        <div style={{ fontSize: '1.125rem', fontWeight: 600, color: 'var(--ep-success)', marginBottom: 8 }}>Aucune erreur détectée</div>
    ...
    <div className="error-list">
      {errors.map((e, i) => (
        <div className="error-item" key={i} style={{ '--item-color': e.quart.color, '--item-bg': e.quart.bg }}>
          {curView !== 'jour' && <div style={{ fontSize: '.75rem', fontWeight: 600, color: 'var(--ep-text-muted)', minWidth: 80 }}>{formatDateShort(e.date)}</div>}
