...
export default function InventaireGlobalPage() {
  ...
  const allRows = useMemo(() => {
    const combined = [...usineRows, ...ghRows];
    combined.forEach((row) => { row._cat = getCategoryForRow(row, categories); });
    return combined;
  }, [usineRows, ghRows, categories]);

  const filteredRows = useMemo(() => {
    return allRows.filter((row) => {
      if (locFilter && row.source !== locFilter) return false;
      if (activeCat === 'NO_CAT') { if (row._cat) return false; }
      else if (activeCat !== null) {
        const cat = categories.find((c) => c.id === activeCat);
        if (!cat) return false;
        if (row.source === 'gh') { if (!matchesCategory(row._original || row, cat)) return false; }
        else {
  ...
  }, [allRows, locFilter, activeCat, search, categories]);

  const kpis = useMemo(() => {
    const rows = filteredRows.length > 0 ? filteredRows : allRows;
    const totalPoids = rows.reduce((s, r) => s + (parseFloat(r.poids_total) || 0), 0);
    return { total: rows.length, usine: rows.filter((r) => r.source === 'usine').length, gh: rows.filter((r) => r.source === 'gh').length, poids: totalPoids };
  }, [filteredRows, allRows]);

  function handleSelectCat(catId) { setActiveCat(catId); }
  function handleSelectCode(code) { setActiveCat(null); setSearch(code); }

  function handleRefresh() {
    setGhRefreshKey((k) => k + 1);
    usineQ.refetch();
    categoriesQ.refetch();
    poidsQ.refetch();
    traxMapQ.refetch();
  ...
  function handleExportPdf() {
    try { exportGlobalPdf(filteredRows, dateReleveUsine, traxMap); addToast('PDF exporté ✓', 'success'); }
    catch (e) { addToast('Erreur PDF : ' + e.message, 'error'); }
  }

  function handleExportExcel() {
    try { exportGlobalExcel(filteredRows, categories, traxMap); addToast('Excel exporté ✓', 'success'); }
    catch (e) { addToast('Erreur Excel : ' + e.message, 'error'); }
  }
  ...
  return (
    <div className="tool-main">
      ...
      <div className="page-sub">Stock combiné de l'usine (relevé le plus récent) et de l'entrepôt GH Logistics. Classé par catégorie, code produit et numéro de lot.</div>

      {noGhData && (
        <div className="info-banner">
          <span>ⓘ</span><span>Stock GH non disponible — connectez-vous à GH Logistics.</span>
        </div>
      )}
      ...
      <div className="toolbar">
        <div className="toolbar-left">
          ...
          <select className="fsel" value={locFilter} onChange={(e) => setLocFilter(e.target.value)}>
