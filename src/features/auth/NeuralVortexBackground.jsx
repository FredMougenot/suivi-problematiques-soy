...
/**
 * Fond animé WebGL "vortex neuronal" — réagit à la position du curseur.
 * Adapté du composant fourni par Fred (background.txt) : suppression des
 * dépendances Next.js (SSR/style jsx), conversion TS → JS, correction d'une
 * fuite d'event listener (touchmove n'était jamais retiré au démontage).
 */
export default function NeuralVortexBackground() {
  ...
  useEffect(() => {
    ...
    const compileShader = (glCtx, source, type) => {
      const shader = glCtx.createShader(type);
      glCtx.shaderSource(shader, source);
      glCtx.compileShader(shader);
      if (!glCtx.getShaderParameter(shader, glCtx.COMPILE_STATUS)) {
        console.error('Erreur shader :', glCtx.getShaderInfoLog(shader));
    ...
    // Position initiale du pointeur au centre pour éviter un flash au coin
  ...
  return <canvas ref={canvasRef} className="lg-canvas" aria-hidden="true"></canvas>;
}
