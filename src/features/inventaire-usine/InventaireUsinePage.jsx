...
export default function InventaireUsinePage() {
  ...
  useEffect(() => {
    if (releveQ.data) {
      if (releveQ.data.length > 0) {
        ...
        setRows(releveQ.data.map((r) => ({
          id: r.id, categorie_id: r.categorie_id || '', categorie_nom: r.categorie_nom || '',
          code_produit: r.code_produit || '', description: r.description || '', no_lot: r.no_lot || '',
          no_sous_lot: r.no_sous_lot || '', quantite: r.quantite ?? '', poids_unit: r.poids_unit ?? '',
          poids_total: r.poids_total ?? '', balance: r.balance ?? '', date_fab: r.date_fab || '',
          date_peremption: r.date_peremption || '', notes: r.notes || '',
        })));
      } else {
        setRows([]);
      }
  ...
  function handleFieldChange(idx, field, value) {
    setRows((prev) => {
      ...
      if (field === 'quantite' || field === 'balance') next[idx].poids_total = calcPoidsTotal(next[idx]);
      if (field === 'code_produit' && value.length >= 3) {
        const cat = matchCategoryForCode(categories, value);
        if (cat) { next[idx].categorie_id = cat.id; next[idx].categorie_nom = cat.name; }
        const poids = getPoidsUnitaire(poidsList, value);
        next[idx].poids_unit = poids || '';
        next[idx].poids_total = calcPoidsTotal(next[idx]);
      }
      return next;
    });
  }

  function handleCategoryChange(idx, id, nom) {
    setRows((prev) => { const next = [...prev]; next[idx] = { ...next[idx], categorie_id: id, categorie_nom: nom }; return next; });
  }

  function handlePickTrax(idx, code, desig) {
    setRows((prev) => {
      const next = [...prev];
      const cat = matchCategoryForCode(categories, code);
      const poids = getPoidsUnitaire(poidsList, code);
      next[idx] = {
        ...next[idx], code_produit: code, description: desig,
        categorie_id: cat ? cat.id : next[idx].categorie_id,
        categorie_nom: cat ? cat.name : next[idx].categorie_nom,
        poids_unit: poids || next[idx].poids_unit,
      };
      next[idx].poids_total = calcPoidsTotal(next[idx]);
      return next;
    });
  ...
  function handleAddRow() {
    setRows((prev) => [...prev, {
      id: nextTmpId(), categorie_id: '', categorie_nom: '', code_produit: '', description: '',
      no_lot: '', no_sous_lot: '', quantite: '', poids_unit: '', poids_total: '', balance: '',
      date_fab: '', date_peremption: '', notes: '',
    }]);
  }
  ...
  function handleNouveauReleve() {
    setConfirmOpen(false);
    setRows([]);
    setDateReleve(getMondayStr());
    setLastSavedDate(null);
    addToast('Relevé réinitialisé. Ajoutez vos produits.', 'info');
  }

  function handleSaveAll() {
    const invalid = rows.filter((r) => !r.code_produit || !r.no_lot);
    if (invalid.length > 0) { addToast(`${invalid.length} ligne(s) incomplète(s) — Code produit et No. lot sont obligatoires.`, 'error'); return; }
    if (!dateReleve) { addToast('Veuillez sélectionner une date de relevé.', 'error'); return; }

    saveMutation.mutate({ dateReleve, rows }, {
      onSuccess: () => { setLastSavedDate(dateReleve); addToast(rows.length + ' produit(s) sauvegardé(s) ✓', 'success'); },
      onError: (e) => addToast('Erreur sauvegarde : ' + e.message, 'error'),
    });
  }
    ...
    <div className="tool-main">
      ...
      <div className="releve-banner">
        <div className="releve-info">
          <div className="releve-date-wrap">
            <div className="releve-date-lbl">Date du relevé</div>
            <div className="releve-date-val">{dateReleve ? fmtDate(dateReleve) : '—'}</div>
          </div>
          ...
          {lastSavedDate && (
            <div style={{ fontSize: '.76rem', color: 'var(--text-muted)' }}>
              Dernier relevé chargé — <span style={{ color: 'var(--text-secondary)', fontWeight: 600 }}>{fmtDate(lastSavedDate)}</span>
            </div>
          )}
        </div>
      ...
      <div className="tbl-wrap">
        {releveQ.isLoading ? (
          ...
          <>
            <SaisieTable
              rows={rows} categories={categories} traxData={traxData}
              onFieldChange={handleFieldChange} onCategoryChange={handleCategoryChange}
              onPickTrax={handlePickTrax} onRemoveRow={handleRemoveRow}
            />
            {rows.length > 0 && (
              <div className="tbl-footer">
                <div className="tbl-totaux">
                  <div className="total-item"><div className="total-lbl">Produits</div><div className="total-val">{totalLignes}</div></div>
                  <div className="total-item"><div className="total-lbl">Qté totale</div><div className="total-val">{totalQte % 1 === 0 ? totalQte : totalQte.toFixed(2)}</div></div>
                  <div className="total-item"><div className="total-lbl">Poids total</div><div className="total-val">{totalPoids.toFixed(2)} kg</div></div>
                </div>
                <div style={{ fontSize: '.72rem', color: 'var(--text-muted)' }}>Poids calculé automatiquement : Qté × Poids unitaire + Balance</div>
              </div>
            )}
          </>
        )}
