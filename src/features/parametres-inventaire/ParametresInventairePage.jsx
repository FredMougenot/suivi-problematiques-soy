...
export default function ParametresInventairePage() {
  ...
  function addRecurrence() {
    const val = parseInt(newRecInput, 10);
    if (isNaN(val) || val < 0) { addToast('Valeur invalide', 'error'); return; }
    if (recurrences.includes(val)) { addToast('Récurrence déjà existante', 'error'); return; }
    const next = [...recurrences, val];
    setRecurrences(next); saveRecs(next); setNewRecInput('');
    addToast(val + 'j ajouté ✓', 'success');
  }

  function removeRec(val) {
    if (rows.some((r) => r.recurrence === val)) { addToast('Impossible — des items utilisent cette récurrence', 'error'); return; }
    const next = recurrences.filter((r) => r !== val);
    setRecurrences(next); saveRecs(next);
    addToast('Récurrence supprimée', 'success');
  }
  ...
  function addRow() {
    const nextNum = rows.length > 0 ? Math.max(...rows.map((r) => r.item_number)) + 1 : 1;
    const defaultRec = sortedRecs.find((r) => r >= 30) ?? recurrences[0] ?? 30;
    setRows((prev) => [...prev, { id: tmpIdCounter--, item_number: nextNum, nom: 'Item ' + nextNum, recurrence: defaultRec, derniere_verification: null, dirty: true, isNew: true }]);
  }
  ...
  async function removeRow(idx) {
    ...
    } catch (e) { addToast('Erreur suppression : ' + e.message, 'error'); }
  }

  async function saveAll() {
    const dirty = rows.filter((r) => r.dirty);
    if (!dirty.length) { addToast('Aucune modification', 'info'); return; }
    setSaving(true);
    let saved = 0, errors = 0;
    for (const r of dirty) {
      try {
        const updated = await saveMutation.mutateAsync(r);
        setRows((prev) => prev.map((row) => (row === r ? updated : row)));
        saved++;
      } catch { errors++; }
    }
    setSaving(false);
    if (errors) addToast(errors + ' erreur(s)', 'error');
    else addToast(saved + ' item(s) sauvegardé(s) ✓', 'success');
  }
    ...
    <div className="tool-main">
      ...
      <div className="rec-section">
        ...
        <div className="rec-chips">
          {sortedRecs.map((r) => (
            <span className="rec-chip" key={r}>
              {r} jour{r > 1 ? 's' : ''}
              {r !== 0 && r !== 1 && <button className="rec-chip-rm" onClick={() => removeRec(r)} title="Supprimer">✕</button>}
            </span>
          ))}
        </div>
      ...
      {paramsQ.isLoading ? (
        ...
        <div className="table-shell param-items-table">
          <table className="data-table">
            ...
            <tbody>
              {rows.map((r, i) => {
                const info = prochaineInfo(r);
                return (
                  <tr key={r.id}>
                    <td>{r.item_number}</td>
                    <td><input className="inp-item" value={r.nom} placeholder="Nom de l'item…" onChange={(e) => markDirty(i, 'nom', e.target.value)} /></td>
                    <td>
                      <select className="sel-rec" value={r.recurrence} onChange={(e) => markDirty(i, 'recurrence', parseInt(e.target.value, 10))}>
                        {sortedRecs.map((rec) => <option key={rec} value={rec}>{rec} jour{rec > 1 ? 's' : ''}</option>)}
                      </select>
                    </td>
