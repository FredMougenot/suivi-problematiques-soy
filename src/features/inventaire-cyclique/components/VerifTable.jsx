  ...
  return (
    <>
      <div className="verif-kpi-bar">
        ...
        {kpis.nUrgent > 0 && <div className="verif-kpi" style={{ background: 'rgba(224,85,85,.1)', border: '1px solid rgba(224,85,85,.3)', color: 'var(--ruby)' }}>{kpis.nUrgent} en urgence</div>}
        {kpis.nSoon > 0 && <div className="verif-kpi" style={{ background: 'rgba(232,164,58,.1)', border: '1px solid rgba(232,164,58,.3)', color: 'var(--amber)' }}>{kpis.nSoon} bientôt à échéance</div>}
        {kpis.nRec > 0 && <div className="verif-kpi" style={{ background: 'rgba(74,158,232,.1)', border: '1px solid rgba(74,158,232,.3)', color: 'var(--sapphire)' }}>{kpis.nRec} recommandé(s)</div>}
          ...
          <tbody>
            {rows.length === 0 ? (
              ...
              rows.map(({ p, i, prochaine, verifAujourdhui, statut, badgeCls, isRec }) => (
                <tr key={p.id} className={isRec ? 'tr-recommended' : ''} style={verifAujourdhui ? { background: 'rgba(45,212,160,.04)' } : undefined}>
                  <td><span className="item-num">{i + 1}</span></td>
                  <td style={{ fontWeight: 500, color: isRec ? 'var(--sapphire)' : undefined }}>{p.item}</td>
                  <td style={{ textAlign: 'center' }}>
                    <span style={{ fontSize: '.72rem', fontWeight: 700, color: REC_COLORS[String(p.recurrence)] || 'var(--text-muted)', padding: '2px 8px', borderRadius: 10, background: 'rgba(255,255,255,.05)' }}>{p.recurrence}j</span>
                  </td>
