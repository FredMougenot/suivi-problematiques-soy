...
import { useParamItemsQuery, useStockQuery, useHistoryQuery, useSaveAllStockMutation, useDeleteStockRowMutation, useSubmitNcMutation } from './queries';
...
export default function InventaireCycliquePage() {
  ...
  useEffect(() => { if (stockQ.data) { setStockRows(stockQ.data); setNextExtra(Math.max(99, ...Object.keys(stockQ.data).map(Number)) + 1); } }, [stockQ.data]);

  const today = localToday();
  const { rows: verifRows, kpis } = useMemo(() => buildVerifRows(paramItems, today), [paramItems, today]);

  function changeDate(delta) { const d = new Date(dateStr + 'T00:00:00'); d.setDate(d.getDate() + delta); setDateStr(d.toISOString().slice(0, 10)); }
  function handleFieldChange(idx, field, value) { setStockRows((prev) => ({ ...prev, [idx]: { ...(prev[idx] || { item_index: idx }), [field]: value } })); }
  function handleAddItem() { const idx = nextExtra; setNextExtra(idx + 1); setStockRows((prev) => ({ ...prev, [idx]: { item_index: idx, label: '' } })); }
  function handleDeleteRow(idx) { if (!confirm('Supprimer cet item ?')) return; deleteRowMutation.mutate(idx, { onSuccess: () => { setStockRows((prev) => { const next = { ...prev }; delete next[idx]; return next; }); addToast('Item supprimé.', 'info'); } }); }
  function handleSaveAll() { const indices = [...BASE_ITEMS.map((it) => it.idx), ...Object.keys(stockRows).map(Number).filter((i) => i >= 100)]; saveAllMutation.mutate({ stockRows, indices, paramItems }, { onSuccess: ({ errors }) => { if (errors > 0) addToast(`${errors} erreur(s).`, 'error'); else addToast('Tout sauvegardé ✓', 'success'); }, onError: (e) => addToast('Erreur : ' + e.message, 'error') }); }
  function handleSubmitNc() { submitNcMutation.mutate(dateStr, { onSuccess: () => addToast('Non-conformité créée ✓', 'success'), onError: (e) => addToast('Erreur : ' + e.message, 'error') }); }
  function handleExportPdf() { exportInventaireCycliquePdf(paramItems); }
  ...
  return (
    <div className="tool-main">
      ...
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 12, padding: '12px 0 4px', flexWrap: 'wrap' }}>
        ...
        <button className="btn btn-danger" onClick={handleSubmitNc}>Soumettre les non-conformités</button>
      </div>
    </div>
  );
}
